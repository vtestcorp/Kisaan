...
public class VendorRegistration_Page {
	...
	public VendorRegistration_Page(WebDriver driver, ExtentTest test) {
		this.driver = driver;
	...
	@FindBy(id = "reg_markets-selectized")
	private WebElement store_TextBox;
	...
	@FindBy(xpath = "//input[@value='sign up']")
	private WebElement registrationSuccessfull_Text;
	...
	public void enter_StoreName(String input_StoreName) throws IOException, InterruptedException {
		applyWait.waitForElementToBeClickable(store_TextBox, DefineConstants.explicitWait_30);
		javascriptClick.highLighterMethod(store_TextBox);
		store_TextBox.click();
		store_TextBox.sendKeys(input_StoreName);
		store_TextBox.sendKeys(Keys.ENTER);
		Screenshots.takeScreenshot(driver, "User entered store name as "+ input_StoreName);
		test.log(Status.INFO, "User entered store name as "+ input_StoreName);
		Log.info("User entered store name as "+ input_StoreName);		
    }
...
//		javascriptClick.highLighterMethod(signUp_Button);
	...
	public void verify_SuccessMessage() throws IOException, InterruptedException {
		scrollTypes.scrollInToView(registrationSuccessfull_Text);
		applyWait.waitForElementToBeClickable(registrationSuccessfull_Text, DefineConstants.explicitWait_30);
		if(registrationSuccessfull_Text.isDisplayed()){
		Assert.assertTrue(true, "Vendor Account created Successfully.");
		javascriptClick.highLighterMethod(registrationSuccessfull_Text);
		Screenshots.takeScreenshot(driver, "Vendor Account created Successfully.");
		test.log(Status.INFO, "Vendor Account created Successfully.");
		Log.info("Vendor Account created Successfully.");		
    }
  }
}
